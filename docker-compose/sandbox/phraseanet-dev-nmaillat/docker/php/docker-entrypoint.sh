...
then
    ...
    make
    bin/setup system:install --db-host=db --db-user=root --db-password=root --email=admin@alchemy-dockerdev.fr --password=admin --appbox=ab_master --databox=db_databox1 --quiet
